// ============================================================
// nlp.js — Advanced Vietnamese NLP Engine v3.0 (Node.js version)
// ============================================================
...
const NLP = {
  ...
  parseDate(text, referenceDate = new Date()) {
    ...
    const dmy = t.match(/(\d{1,2})[\/\-](\d{1,2})(?:[\/\-](\d{4}|\d{2}))?/);
    if (dmy) {
    ...
    const dtm = t.match(/(?:ngay\s*)?(\d{1,2})\s*[\/\-]?\s*thang\s*(\d{1,2})(?:\s*(?:nam\s*)?(\d{4}))?/i);
    if (dtm) {
    ...
    const moOnly = t.match(/thang\s*(\d{1,2})(?:\s*[\/\-]\s*(\d{4}))?/i);
    if (moOnly) {
  ...
  extractAllDates(text) {
    ...
    const range = t.match(/(?:tu|from)\s+(.+?)\s+(?:den|toi|den ngay|to)\s+(.+?)(?:\s|$|,|\.)/i);
    if (range) {
    ...
    const co = t.match(/(?:check.?out|tra phong|ra)\s+(?:ngay\s*)?(.+?)(?:\s|$|,)/i);
    const ci = t.match(/(?:check.?in|nhan phong|vao|tu)\s+(?:ngay\s*)?(.+?)(?:\s|$|,)/i);
    if (ci && co) {
    ...
    const tokens = t.split(/\s+(?:den|toi|va|→|-|,)\s+/);
    if (tokens.length >= 2) {
  ...
  extractNights(text) {
    if (!text) return null;
    const m = text.match(/(\d+)\s*(dem|ngay|nights?|days?)/i);
    if (m) return parseInt(m[1]);
    // Word numbers: "hai đêm"
    const wn = text.match(/(mot|hai|ba|bon|nam|sau|bay|tam|chin|muoi)\s*(dem|ngay)/i);
    if (wn) {
      const map = { 'mot':1,'hai':2,'ba':3,'bon':4,'nam':5,'sau':6,'bay':7,'tam':8,'chin':9,'muoi':10 };
  ...
  extractGuests(text) {
    ...
    const mixed = t.match(/(\d+)\s*(?:nguoi lon|adult)/i);
    const kids = t.match(/(\d+)\s*(?:tre em|be|em be|kids?|children?)/i);
    if (mixed) {
    ...
    const simple = t.match(/(\d+)\s*(?:nguoi|khach|pax|adults?|members?|ban|nguoi ban)/i);
    if (simple) return { adults: parseInt(simple[1]), children: 0, total: parseInt(simple[1]) };

    // Number word: "hai người"
    const wordNum = t.match(/(mot|hai|ba|bon|nam|sau|bay|tam|chin|muoi)\s*(?:nguoi|khach|ban)/i);
    if (wordNum) {
      const map = { 'mot':1,'hai':2,'ba':3,'bon':4,'nam':5,'sau':6,'bay':7,'tam':8,'chin':9,'muoi':10 };
    ...
    if (t.match(/ca nha|gia dinh|family/i)) return { adults: 4, children: 1, total: 5, inferred: true };
    if (t.match(/cap doi|couple|2 vo chong|anh em|2 anh|chung minh 2/i)) return { adults: 2, children: 0, total: 2, inferred: true };
    if (t.match(/nhom ban|hoi ban|team building/i)) return { adults: 6, children: 0, total: 6, inferred: true };
  ...
  extractBudget(text) {
    ...
    const m = t.match(/(?:duoi|khong qua|tam|khoang|budget|gia tam|gia khoang|gia khong|toi da)?\s*([\d,.]+)\s*(tr|trieu|k|nghin|dong|vnd)/i);
    if (!m) return null;
    let val = parseFloat(m[1].replace(/,/g, '.'));
    const unit = m[2].toLowerCase();
    if (unit === 'tr' || unit === 'trieu') val *= 1000000;
    else if (unit === 'k' || unit === 'nghin') val *= 1000;
    const isMax = !!t.match(/duoi|khong qua|toi da|toi thieu|budget/i);
  ...
  extractRoomPreferences(text) {
    ...
    const t = text.toLowerCase();
    const prefs = {
      wantsView: t.match(/view|canh|nhin ra|ngam/i) ? true : null,
      wantsBathtub: t.match(/bon tam|bathtub|bon/) ? true : null,
      wantsPool: t.match(/ho boi|be boi|pool/) ? true : null,
      wantsKitchen: t.match(/bep|nau an|kitchen/) ? true : null,
      wantsHighFloor: t.match(/tang cao|view cao|tren cao/) ? true : null,
      occasion: null,
    };
    if (t.match(/honeymoon|tuan trang mat|wedding|le cuoi|ky niem/)) prefs.occasion = 'honeymoon';
    else if (t.match(/sinh nhat|birthday/)) prefs.occasion = 'birthday';
    else if (t.match(/team building|cong ty|company/)) prefs.occasion = 'corporate';
    else if (t.match(/family|gia dinh|ca nha/)) prefs.occasion = 'family';
    return prefs;
  },
  ...
  extractBranch(text) {
    if (!text) return null;
    const n = this._norm(text);
    if (n.match(/\b(tan binh|xuan hong|cs1|co so 1)\b/)) return { key: 'cs1', name: 'Tân Bình' };
    if (n.match(/\b(quan 10|q10|cs2|co so 2|3 thang 2)\b/)) return { key: 'cs2', name: 'Quận 10' };
    if (n.match(/\b(quan 5|q5|cs3|co so 3|pham huu chi)\b/)) return { key: 'cs3', name: 'Quận 5' };
    if (n.match(/\b(go vap|cs4|co so 4|phan huy ich)\b/)) return { key: 'cs4', name: 'Gò Vấp' };
    if (n.match(/\b(binh thanh|cs5|co so 5|bui dinh tuy)\b/)) return { key: 'cs5', name: 'Bình Thạnh' };
    return null;
  },
  ...
  detectIntent(text) {
    ...
    if (n.match(/\b(tu|from)\b/) && n.match(/\b(den|toi|to)\b/) && (n.match(/\b(cs1|cs2|cs3|cs4|cs5|tan binh|quan 10|quan 5|go vap|binh thanh)\b/) || n.match(/\b(dem|ngay)\b/)))
      return 'booking_inquiry';

    // Mentions of branch names alone
    if (n.match(/\b(cs1|cs2|cs3|cs4|cs5|tan binh|quan 10|quan 5|go vap|binh thanh|xuan hong|3 thang 2|pham huu chi|phan huy ich|bui dinh tuy)\b/) && !n.match(/\b(pass|password|wifi|mat khau|o to|xe may|xe dap|an uong|nha hang|gan day|cach|km|parking|bai do|don san bay|check.?in|check.?out|mat dien|nuoc khong|mat nuoc|hong|hu|su co)\b/))
      return 'booking_inquiry';
...
Schema:
{
  ...
  "branch": "cs1|cs2|cs3|cs4|cs5 hoặc null",
  "branch_name": "Tân Bình|Quận 10|Quận 5|Gò Vấp|Bình Thạnh hoặc null",
...
Tin nhắn khách: "${text}"`;
    ...
    try {
      ...
      console.warn('[NLP] Gemini error, falling back to rule-based parsing:', e.message);
    }
    return null;
  }
};
